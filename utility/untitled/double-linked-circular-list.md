...
[https://github.com/ddzy/ts-utility-plugins/tree/master/src/ddzy/utility/algorithm/double-linked-circular-list](https://github.com/ddzy/ts-utility-plugins/tree/master/src/ddzy/utility/algorithm/double-linked-circular-list)

### 用法

{% code title="doubleLinkedCircularList.test.ts" %}
```typescript
...
| :--- | :--- | :--- |
| handleGetHead | \(\) =&gt; ListNode \| null | 获取头节点 |
| handleGetTail | \(\) =&gt; ListNode \| null | 获取尾节点 |
| handleAppend | \(value: V\) =&gt; DoubleLinkedCircularList&lt;V&gt; | 尾部追加节点 |
| handlePrepend | \(value: V\) =&gt; DoubleLinkedCircularList&lt;V&gt; | 首部追加节点 |
| handleInsertBefore | \(target: V, value: V\) =&gt; DoubleLinkedCircularList&lt;V&gt; | 将目标节点插入至源节点前 |
| handleInsertAfter | \(target: V, value: V\) =&gt; DoubleLinkedCircularList&lt;V&gt; | 将目标节点插入至源节点后 |
| handleTraversalWithForward | \(callback: \(node: ListNode&lt;V&gt;\) =&gt; void\) =&gt; void | 正向遍历双向循环链表 |
| handleTraversalWithBackward | \(callback: \(node: ListNode&lt;V&gt;\) =&gt; void\) =&gt; void | 反向遍历双向循环链表 |
| handleGetLength | \(\) =&gt; number | 获取链表长度 |
| handleRemove | \(value: V\) =&gt; DoubleLinkedCircularList | 移除链表指定节点 |
